...
#include<string.h>
#include<time.h>
//NOTE:khai bao mot so bien su dung cho chuong trinh
int character[1000];
int length = 2;
int fruit;
int score;
...
//NOTE:random ngau nhien vi tri moi
...
void information(){
	...
	gotoxy(3,10);
	printf("chuc ae choi game vui ve, an phim bat ki de tro lai menu");
	gotoxy(27,11);
	SetColor(14);
	printf("----Tran_anh----");
	getchar();
...
//NOTE:ham cai dat toc do cho con ran
void setting(){
  system("cls");
  gotoxy(0,9);
...
//NOTE:ham menu hien khi bat dau chay chuong trinh
void menu(){
	SetColor(113);
	system("cls");
    gotoxy(25,3);
...
//NOTE:ham khoi tao cac gia tri ban dau
void initgame(){
	system("cls");
	s_e = 120-speed;
	t = 0;
...
//NOTE:ham xuat hien moi theo vi tri ngau nhien
void creatfruit(){
	int notok = 1;
	while(notok){
		notok = 0;
...
//NOTE:ham gop toa do 2 so ve 1 so
int getx(int a){
	return (a-10000)/100;
}
int gety(int a){
...
//NOTE:ham chinh cua chuong trinh
int main(){
	state = MENU;
    menu();
while(1){
